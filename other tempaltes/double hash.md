...
- `sz` is the maximum size of the string. first you have to make sure it is right
- then you have to call the `calculate_pow()` function
- then you can create an object of the `HASH` class, and call functions using that object. 
- complexity of `create_prefix_hash_table()` and  `hash_total_string()` is `O(n)`. And complexity of `hash_val()` id `O(1)`.
